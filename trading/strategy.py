    ...
    for i in range(6):
        ...
        for j in range(len(coin_tickers)):
         #일일 거래 횟수
            ...
            price_now = pyupbit.get_current_price(coin_tickers[j])                                     # line test X
            ...
            if price_now >= price_pre24 or price_now < price_pre24*0.9:                          # line test X
                if money_share[j][i] == 0:
                    ...
                    buy_info = upbit.buy_market_order(coin_tickers[j], math.floor((buy_amount/price_now)*10**8)/10**8)   
                ...
                if money_share[j][i] == 0:
                    ...
                    sell_info = upbit.sell_market_order(coin_tickers[j],sell_amount)
                    money_share[j][i] = 1
                    coin_num[j][i] = 0
                    #판매
                else:
